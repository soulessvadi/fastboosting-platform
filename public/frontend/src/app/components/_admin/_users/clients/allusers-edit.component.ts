...
@Component({
  selector: 'user-allusers-edit',
  templateUrl: './allusers-edit.component.html',
})

export class AllUsersEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this._service.user_types.then((types: any) => this.types = types);
  ...
  public save() {
    ...
    this._service.saveUser(this.user.id, data).subscribe((res: any) => {
  ...
  public fetch() {
    this._service.getUser(this.user_id).subscribe((res: any) => {
      this.user = res.body.user;
      this.countries = res.body.countries;
      this.orders = res.body.orders;
      this.ordersc = res.body.ordersc;
